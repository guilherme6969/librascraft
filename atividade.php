...
$consulta = "SELECT id_palavra,palavra, video_sinal FROM palavra WHERE cod_subfase = $pagina 
				AND id_palavra NOT IN (
					SELECT cod_palavra FROM resposta_ouvinte WHERE cod_usuario='".$_SESSION["autorizado"]."'
				) 
...
if($qtd>0){
	...
	if($_GET["pagina"]=="7" || $_GET["pagina"]=="8"){
		...
		$atividade= '<iframe id="link_video" width="500" height="300" src="https://www.youtube.com/embed/'.$linha_questao["video_sinal"].'?rel=0&modestbranding=1&autohide=1&showinfo=0&controls=0" class="rounded mx-auto d-block" frameborder="0" allow="accelerometer; autoplay; encrypted-media; gyroscope; picture-in-picture" allowfullscreen></iframe>';
					...
					<div class="col d-flex flex-column justify-content-center align-items-center">
						...
						<div class="row">
							...
							<?php
								...
								//quantas respostas de palavras foram realizadas
								$select = "SELECT COUNT(id_resposta_ouvinte) as qtd FROM resposta_ouvinte 
													WHERE cod_subfase='".$_GET["pagina"]."'";
								$resultado = mysqli_query($conexao,$select)
											or die(mysqli_error($conexao));
											$linha = mysqli_fetch_assoc($resultado);
											$qtd_questoes = $linha["qtd"];															
								
								//quantas respostas foram acertadas
								$select = "SELECT COUNT(id_resposta_ouvinte) as acertos FROM resposta_ouvinte 
								WHERE cod_subfase='".$_GET["pagina"]."' AND resposta=cod_palavra";
								$resultado = mysqli_query($conexao,$select)
									or die(mysqli_error($conexao));
									$linha = mysqli_fetch_assoc($resultado);
									$qtd_acertos = $linha["acertos"];				
									
								header("location: score.php?pagina=".$_GET["pagina"]);
								//porcentagem de acerto
								$porcentagem = number_format((100*($qtd_acertos/$qtd_questoes)),2);
								echo "Você acertou $qtd_acertos de $qtd_questoes ($porcentagem%).";
								//se maior ou igual que 75%, ele passou a subfase
								if($porcentagem>=75){
									echo "<h3>Você passou desta fase! Parabéns!";
									echo "<a href='mapa.php'>Voltar ao MAPA!</a>";
								}
								//senao ele reprovou a subfase e deve zerar as respostas para começar denovo.
								else{
									echo "<h3>Infelizmente você não atingiu a nota mínima para esta fase (75% de acerto).<br />";
									echo "<a href='limpar_respostas.php?pagina=".$_GET["pagina"]."'>Refazer tarefa</a>";
								}
							}
							?>
	...
	$(function(){
		$(".resposta").click(function(){
			...
			$.post("salva_resposta.php",post,function(r){
				console.log(r);
				if(r=="1"){					
					location.href='atividade.php?pagina='+sf ;
				}
				else{
					console.log(r);
				}
